---
title: "Divide and Conquer Algorithm —— Version 2"
output:
...
path <- paste(getwd(), '/DC_data_v2/', sep = '')
...
partition <- 1000  # Each distribution generate 50,000 samples in each worker
...
Save the spited data in Workers Level
...
for(current_worker in name_workers){
  ...
  fx <- function(dpCluster) Fit(dpCluster, 1000, progressBar = FALSE)
  dpCluster <- mclapply(dpCluster, fx, mc.cores = detectCores())
  plot(dpCluster)
